...
void MysqlServer::sync(const char * id)
{
	shop_id = id;
	while (!connectdata());
	while (!sqlconnect());
...
bool MysqlServer::goodsupdate(MYSQL *mysql, const MYSQL_ROW record)
{
	...
	if (!strcmp("update", record[2]))//update method
	{
		...
		if (mysql_query(mysql, sql.c_str()))
		{
			...
			{
				std::string tmp;
				sql = "update goods set name = '";
				tmp = record[1];
				sql += temp + "' and price = ";
				tmp = record[2];
				sql += tmp + " and score = ";
				tmp = record[3];
				sql += tmp + " and discount = ";
				tmp = record[4];
				sql += tmp + " and brand = '";
				tmp = record[5];
				sql += tmp + "' where cord = '";
				tmp = record[6];
				sql += tmp + "' and shop_id = " + id + ";";
		...
		if (!(strcmp("insert", record[2])))//insert method
		{
			...
			if (mysql_query(mysql, sql.c_str()))
			{
				...
				{
					std::string tmp;
					sql = "insert into goods(code,name,price,score,discount,brand) values( '";
					tmp = record[0];
					sql += temp + "','";
					tmp = record[1];
					sql += tmp + "',";
...
bool MysqlServer::casherupdate(MYSQL *mysql,MYSQL_ROW record)
{
	...
	if (!strcmp("insert", record[2]))//insert method
	{
		sql = "select id, password from casher where shop_id = " + id + " and id = " + temp + ";";
		if (mysql_query(mysql, sql.c_str()))
		{
			MYSQL_RES * res = mysql_store_result(mysql);
			MYSQL_ROW record = mysql_fetch_row(res);//因为只有一个结果
